...
class FineTuningsApi
{
     ...
     */
    public function generateImagePrompt($workspace_id, string $contentType = self::contentTypes['generateImagePrompt'][0])
    {
        list($response) = $this->generateImagePromptWithHttpInfo($workspace_id, $contentType);
        return $response;
    }
     ...
     */
    public function generateImagePromptWithHttpInfo($workspace_id, string $contentType = self::contentTypes['generateImagePrompt'][0])
    {
        $request = $this->generateImagePromptRequest($workspace_id, $contentType);
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\FlowHunt\Model\ImagePromptResponse' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\FlowHunt\Model\ImagePromptResponse' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\FlowHunt\Model\ImagePromptResponse', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                case 422:
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function generateImagePromptAsync($workspace_id, string $contentType = self::contentTypes['generateImagePrompt'][0])
    {
        return $this->generateImagePromptAsyncWithHttpInfo($workspace_id, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function generateImagePromptAsyncWithHttpInfo($workspace_id, string $contentType = self::contentTypes['generateImagePrompt'][0])
    {
        $returnType = '\FlowHunt\Model\ImagePromptResponse';
        $request = $this->generateImagePromptRequest($workspace_id, $contentType);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function generateImagePromptRequest($workspace_id, string $contentType = self::contentTypes['generateImagePrompt'][0])
    {
        ...
        $resourcePath = '/v2/fine_tunings/inference/images/generate-prompt';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
