...
| [**serpClusterBulkDeleteQueries()**](SERPApi.md#serpClusterBulkDeleteQueries) | **DELETE** /v2/serp/cluster/{group_id}/bulk_delete | Serp Cluster Bulk Delete Queries |
...
```php
serpClusterBulkDeleteQueries($group_id, $workspace_id, $serp_query_request): \FlowHunt\Model\Completed
```
...
$group_id = 'group_id_example'; // string
$workspace_id = 'workspace_id_example'; // string
$serp_query_request = array(new \FlowHunt\Model\SerpQueryRequest()); // \FlowHunt\Model\SerpQueryRequest[]

try {
    $result = $apiInstance->serpClusterBulkDeleteQueries($group_id, $workspace_id, $serp_query_request);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling SERPApi->serpClusterBulkDeleteQueries: ', $e->getMessage(), PHP_EOL;
}
```
